...
const storyNames: string[] = ["Jungle", "Space", "Ocean", "Desert", "Forest", "Castle"];
        ...
        <div className="flex justify-center items-center sm:h-screen max-sm:p-6">
            <div className="grid grid-cols-2 gap-6 w-full sm:grid-cols-3">
                {storyNames.map((storyName, index) => {
                    ...
                    return (
                        <Link key={index} href={storyUrl} className="bg-lightGray rounded-40px aspect-square flex justify-center items-center font-medium text-4xl font-Manrope">
                            {storyName}
                        </Link>
                    );
                })}
