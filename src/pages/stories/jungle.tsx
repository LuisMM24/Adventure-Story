...
function Jungle(props: Props) {
    ...
    return (
        <ContentContainer gapSpacing="gap-[50px]" parentClassName="bg-blurry-temple bg-cover">
            <ElDoradoTitleSVG />
            <div className="relative flex justify-center items-center">
                <ElDoradoMapSVG className="w-[80vw]" />
                {levels.map((level, index) => {
                    const previousLevelData = user.jungleStory.levels.find((i) => i.id === level.level - 1);
                    const isPlayable = previousLevelData?.success || level.level === 1 ? true : false;

                    return <Level key={index} playable={isPlayable} {...level} />;
                })}
            </div>
        </ContentContainer>
    );
